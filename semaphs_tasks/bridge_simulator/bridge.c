...
void Car_method(int sem_id, int index, struct Bridge_organization* shm_struct)
{
    ...
    //! Start working with shared variables in struct
    P_oper(sem_id, SHRD_VAR, 1);

    (shm_struct->num_of_waiting_cars)++;
    printf("At the moment %d in queue of cars!\n\n", (shm_struct->num_of_waiting_cars)++);
    printf("At the moment driver[%d] stay in queue!\n\n", index);

    V_oper(sem_id, SHRD_VAR, 1);
    //! Driver[index] end of work with shared variables in struct
    ...
    //! Driver[index] start working with shared variables in struct
    P_oper(sem_id, SHRD_VAR, 1);
    (shm_struct->num_of_waiting_cars)--;
    V_oper(sem_id, SHRD_VAR, 1);
    ...
    if (( (shm_struct->num_of_waiting_ships != 0) && (shm_struct->num_of_waiting_cars == 0) ) || ( shm_struct->num_of_waiting_ships > 3 ))
    {
        //! Ship could move
...
void Ship_method(int sem_id, int index, struct Bridge_organization* shm_struct)
{
    ...
    printf("At the moment %d in queue of ships!\n\n", shm_struct->num_of_waiting_ships++);
    ...
    //! There is checking for possibility moving of cars or ships
    if ((shm_struct->num_of_waiting_ships) != 0)
    {
        //! Ship should move
        V_oper(sem_id, SHIP, 1);
